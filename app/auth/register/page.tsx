...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setServerError(null);
    try {
      await registerUser({ email: data.email, username: data.name, password: data.password });
      ...
      >
        ...
        <div className="flex-1 flex flex-col justify-center px-8 py-10 sm:px-12 md:px-14 overflow-y-auto">
          <div className="w-full max-w-sm mx-auto">
            ...
            {/* Header */}
            <div className="mb-6">
              <h1 className="text-2xl sm:text-[28px] font-bold text-gray-900 tracking-tight">Create Account 🚀</h1>
              <p className="mt-1.5 text-sm text-gray-400">Sign up to get started on your journey.</p>
            </div>

            {serverError && (
              <div className="mb-4 rounded-xl bg-red-50 border border-red-100 px-4 py-3 text-sm text-red-500">{serverError}</div>
            )}

            {/* Google Sign Up — at top for prominence */}
            <button
              type="button"
              onClick={handleGoogleLogin}
              className="w-full flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-gray-50 py-3 text-sm font-medium text-gray-600 hover:bg-white hover:border-gray-300 hover:shadow-sm transition-all duration-200 mb-5"
            >
              <GoogleIcon />
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input {...register("name")} placeholder="Enter your name" autoComplete="name"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input {...register("address")} placeholder="Enter your address" autoComplete="street-address"
                    className="w-full rounded-xl border border-gray-200 bg-gray-50 pl-11 pr-4 py-3 text-sm text-gray-800 placeholder-gray-300 focus:outline-none focus:bg-white focus:border-rose-300 focus:ring-2 focus:ring-rose-100 transition-all duration-200" />
                </div>
                {errors.address && <p className="text-xs text-red-400 pl-1">{errors.address.message}</p>}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} {...register("password")} placeholder="••••••••" autoComplete="new-password"
                  ...
                  <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-rose-400 transition-colors">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Lock className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 w-4 h-4" />
                  <input type={showConfirmPassword ? "text" : "password"} {...register("confirmPassword")} placeholder="••••••••" autoComplete="new-password"
                    className="w-full rounded-xl border border-gray-200 bg-gray-50 pl-11 pr-11 py-3 text-sm text-gray-800 placeholder-gray-300 focus:outline-none focus:bg-white focus:border-rose-300 focus:ring-2 focus:ring-rose-100 transition-all duration-200" />
                  <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-rose-400 transition-colors">
            ...
            <p className="mt-5 text-center text-sm text-gray-400">
              ...
              <Link href="/auth/login" className="text-rose-500 hover:text-rose-600 font-semibold transition-colors">Sign in</Link>
            </p>
          </div>
        </div>
      </motion.div>
